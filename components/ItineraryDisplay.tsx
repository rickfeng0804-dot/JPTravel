...
import { MapPin, ArrowLeft, ShoppingBag, Gift, Utensils, Camera, Train, Bed, ChevronDown, ChevronUp, BookOpen, Download, Share2, Loader2, Sparkles, Image as ImageIcon, FileSpreadsheet } from 'lucide-react';
...
const ItineraryDisplay: React.FC<ItineraryDisplayProps> = ({ itinerary, onReset }) => {
  ...
  useEffect(() => {
    const handleBeforePrint = () => setIsSummaryOpen(true);
    ...
    <div className="w-full max-w-4xl mx-auto space-y-8 pb-12 animate-fade-in-up print:space-y-4 print:pb-0">
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-white/80 backdrop-blur rounded-2xl p-4 shadow-sm border border-emerald-50 print:hidden">
        <div className="flex w-full md:w-auto justify-between items-center">
            ...
            <button onClick={handleExportExcel} className="md:hidden flex items-center text-emerald-600 bg-emerald-50 hover:bg-emerald-100 px-3 py-1.5 rounded-lg font-medium text-sm transition-colors">
      ...
      <div className="bg-white/90 rounded-3xl overflow-hidden shadow-lg border border-emerald-50 mb-8 print:border-2 print:bg-white">
        ...
        <div className={`p-8 pt-6 text-center border-t border-emerald-50/50 animate-fade-in print:block ${isSummaryOpen ? 'block' : 'hidden'}`}>
            ...
            {/* Cover Image Section */}
            {/* Modified: Only hidden in print if no image exists. If image exists, we want to print it. */}
            ...
            {/* Default Title (Show only if no cover image or when printing) */}
            {(!coverImage) && (
              <div className="mb-6">
            ...
            {/* Print Header Logic - Hide this standard header if we have a fancy cover image in print mode */}
      ...
      {/* Footer Lists */}
      <div className="grid md:grid-cols-2 gap-8 mt-12 print:grid-cols-1">
